...
## Candidate Experiment: Courier-Overlap Pair Grouping
- Hypothesis: Wang/Tsinghua-style instant-delivery optimization suggests separating order-combination candidate generation from downstream assignment. The exact 710.71 floor mostly scores a pair by direct pair potential; it may miss pairs whose two singleton orders share the same strong courier neighborhood, especially in high-courier-pool medium/large cases.
- Diagnostic tooling: add `scripts/overlap_group_probe.py` to sweep offline overlap-weighted pair matchings without touching the submitted path. The probe computes top single-order couriers, adds a shared-courier overlap bonus to the existing `potential_half` edge score, and compares the resulting grouping against the 710.71 output under `prop/seq/uniform/submean`.
- Code change: add `_make_overlap_grouping()` and call only four overlap seeds before the old low-willingness branch. The branch is tightly gated to `tm == 'prop'`, non-extreme-low cases, `25 <= n_tasks <= 45`, and at least `70` couriers; outputs are accepted only through the existing internal evaluator. This avoids the detected `medium_seed201` regression at 60 couriers and keeps `low_willingness_seed501` hash-stable.
- Local evidence: full hidden-like guard vs `baseline_71071.py` keeps high-noise, large301, low, medium201, medium202, scarce, small, and tiny hash-stable. It improves `large_seed302` (`prop 671.294 -> 660.217`, `seq 602.709 -> 586.823`) and `medium_seed203` (`prop 652.324 -> 633.352`, `seq 597.316 -> 582.796`). Full calibrated guard keeps all non-target rows stable and improves `large_seed302` (`prop 660.665 -> 655.865`, `seq 598.868 -> 574.876`) and `medium_seed203` (`prop 583.628 -> 571.977`, `seq 578.401 -> 551.539`).
- Stability evidence: repeat3 on both changed rows is deterministic (`large_seed302` hash stable, `medium_seed203` hash stable) with no runtime increase in the local harness. `submission.py` is `97,694` bytes, under the 100KB limit. Python 3.6 AST parse and all 23 unit tests pass.
- Risk: the previous pair-rematching family failed online despite local proxy gains. This branch is narrower and candidate-generation-only, not a late fixed-mask rematcher, but it still needs one online validation before being treated as a new floor.
